...
describe "MongoMapper::Plugins::MultiParameterAttributes" do
  ...
  it "should assign date with empty year" do
    ...
    @topic.last_read.to_date.should == Date.new(@now.year, 6, 24)
  end
  ...
  it "should assign date with empty month" do
    ...
    @topic.attributes = attributes
    @topic.last_read.to_date.should == Date.new(2004, @now.month, 24)
  end
  ...
  it "should assign date with empty day" do
    ...
    @topic.attributes = attributes
    @topic.last_read.to_date.should == Date.new(2004, 6, @now.day)
  end
  ...
  it "should assign date with empty day and year" do
    ...
    @topic.last_read.to_date.should == Date.new(@now.year, 6, @now.day)
  end
  ...
  it "should assign date with empty day and month" do
    ...
    @topic.attributes = attributes
    @topic.last_read.to_date.should == Date.new(2004, @now.month, @now.day)
  end
  ...
  it "should assign time with and empty date" do
    attributes = {
      "written_on(1i)" => "", "written_on(2i)" => "", "written_on(3i)" => "",
      "written_on(4i)" => "16", "written_on(5i)" => "24", "written_on(6i)" => "10"
    }

    @topic.attributes = attributes
    @topic.written_on.should == Time.zone.local(@now.year, @now.month, @now.day, 16, 24, 10)
  end
